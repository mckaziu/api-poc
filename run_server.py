...
parser.add_argument("-a", "--api", dest="api", choices=['simple', 'flask-plain', 'flask-restful', 'fastapi'], required=True, help="select version of the server and API", metavar="FILE")
parser.add_argument("-p", "--port", dest="port", type=int, required=True, help="select port on which server should run")
